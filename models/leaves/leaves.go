...
func ListLeaveApproval(condArr map[string]string, page int, offset int) (num int64, err error, ops []Leaves) {
	...
	qb.Select("l.leaveid", "l.type", "l.userid", "l.started", "l.ended", "l.days", "l.approverids", "la.status").From(models.TableName("leaves_approver") + " AS la").
		LeftJoin(models.TableName("leaves") + " AS l").On("l.leaveid = la.leaveid").
		Where("la.userid=?").
		And("l.status=2")
...
func CountLeaveApproval(condArr map[string]string) int64 {
	qb, _ := orm.NewQueryBuilder("mysql")
	qb.Select("Count(1) AS num").From(models.TableName("leaves_approver") + " AS la").
		LeftJoin(models.TableName("leaves") + " AS l").On("l.leaveid = la.leaveid").
		Where("la.userid=?").
		And("l.status=2")
	if condArr["status"] == "0" {
