...
func ListOvertimeApproval(condArr map[string]string, page int, offset int) (num int64, err error, ops []Overtimes) {
	...
	qb.Select("l.overtimeid", "l.userid", "l.started", "l.ended", "l.longtime", "l.holiday", "l.way", "l.approverids", "la.status").From(models.TableName("overtimes_approver") + " AS la").
		LeftJoin(models.TableName("overtimes") + " as l").On("l.overtimeid = la.overtimeid").
		Where("la.userid=?").
		And("l.status=2")
...
func CountOvertimeApproval(condArr map[string]string) int64 {
	qb, _ := orm.NewQueryBuilder("mysql")
	qb.Select("Count(1) as num").From(models.TableName("overtimes_approver") + " as la").
		LeftJoin( models.TableName("overtimes") + " as l").On("l.overtimeid = la.overtimeid").
		Where("la.userid=?").
		And("l.status=2")
	if condArr["status"] == "0" {
