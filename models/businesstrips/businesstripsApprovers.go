...
func ListBusinesstripApproverProcess(businesstripid int64) (num int64, err error, user []BusinesstripApproverProcess) {
	...
	qb.Select("upr.userid", "upr.realname", "p.name AS position", "u.avatar", "la.status", "la.summary", "la.changed").From(models.TableName("businesstrips_approver") + " AS la").
		LeftJoin(models.TableName("users") + " AS u").On("u.userid = la.userid").
