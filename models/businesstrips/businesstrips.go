...
func ListBusinesstripApproval(condArr map[string]string, page int, offset int) (num int64, err error, ops []Businesstrips) {
	...
	qb.Select("l.businesstripid", "l.userid", "l.days", "l.changed", "l.approverids", "la.status").From(models.TableName("businesstrips_approver") + " AS la").
		LeftJoin(models.TableName("businesstrips") + " as l").On("l.businesstripid = la.businesstripid").
		Where("la.userid=?").
		And("l.status=2")
...
func CountBusinesstripApproval(condArr map[string]string) int64 {
	qb, _ := orm.NewQueryBuilder("mysql")
	qb.Select("Count(1) AS num").From(models.TableName("businesstrips_approver") + " as la").
		LeftJoin(models.TableName("businesstrips") + " as l").On("l.businesstripid = la.businesstripid").
		Where("la.userid=?").
		And("l.status=2")
	if condArr["status"] == "0" {
