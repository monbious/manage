...
func ListExpenseApproval(condArr map[string]string, page int, offset int) (num int64, err error, ops []Expenses) {
	...
	qb.Select("l.expenseid", "l.userid", "l.total", "l.changed", "l.approverids", "la.status").From(models.TableName("expenses_approver") + " as la").
		LeftJoin(models.TableName("expenses") + " as l").On("l.expenseid = la.expenseid").
		Where("la.userid=?").
		And("l.status=2")
...
func CountExpenseApproval(condArr map[string]string) int64 {
	qb, _ := orm.NewQueryBuilder("mysql")
	qb.Select("Count(1) as num").From(models.TableName("expenses_approver") + " as la").
		LeftJoin(models.TableName("expenses") + " as l").On("l.expenseid = la.expenseid").
		Where("la.userid=?").
		And("l.status=2")
	if condArr["status"] == "0" {
